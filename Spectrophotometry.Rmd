...
Find concentrations of plant extract. By looking at the equation of our standard curve, we know the concentrations of BSA and their absorbance. Our goal is to find the concentrations of our plant extract (x values in the equation), based on the absorbance rates we measured (y values).
Therefore, in the equation:  
_y = 0.33 + 0.0179 * x_,  
the known is _y_, and we want to solve for the unknown _x_ (concentration of plant extract):  
_x = (y - 0.33)/0.0179_
```{r}
...
Convert dilutions of our samples back to the true values.
...
Note: at this step we exclude plant samples with the ratio of 1/10 as they values were too high to be used with our standard curve. 
```{r}
